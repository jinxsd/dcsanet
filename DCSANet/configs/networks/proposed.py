...
from models.backbones.my_attention import DPAM
from models.necks.my_fpn import SR_EnhancedFPN_P2
...
class AttentionResNetBackbone(ResNetBackbone):
    def __init__(self, arch, norm_layer, freeze_indices):
        super().__init__(arch=arch, norm_layer=norm_layer, freeze_indices=freeze_indices)
        self.attention_modules = nn.ModuleList([
            DPAM(in_channels) for in_channels in self.num_channels
        ])
    ...
    def forward(self, x):
        ...
        for i, feature in enumerate(features):
            if feature is not None:
                features[i] = self.attention_modules[i](feature)
        return features
...
DCSA_roi_heads = RoIHeads(
        box_roi_pool=MultiScaleRoIAlign(featmap_names=backbone.return_layers, output_size=7, sampling_ratio=2),
