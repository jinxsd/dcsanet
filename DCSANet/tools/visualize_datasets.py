...
def parse_args():
    ...
    parser.add_argument("--category-colors", type=str, default=None,
                       help="类别颜色映射，格式: '0:255,0,0;1:0,255,0;...'")

    # visualize parameters
    parser.add_argument("--show-dir", type=str, default=None, required=True)
    parser.add_argument("--show-conf", type=float, default=0.001)

    # plot parameters
    parser.add_argument("--font-scale", type=float, default=1.0)
    parser.add_argument("--box-thick", type=int, default=2)
    ...
    if args.category_colors:
            category_colors = {}
            for item in args.category_colors.split(';'):
                if ':' in item:
                    cat_id, color_str = item.split(':', 1)
                    try:
                        color = tuple(map(int, color_str.split(',')))
                        category_colors[int(cat_id)] = color
                    except ValueError:
                        print(f"忽略无效的颜色格式: {item}")
            args.category_colors = category_colors
    else:
